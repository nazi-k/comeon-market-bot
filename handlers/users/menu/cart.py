...
async def send_edit_cart_menu(call: types.CallbackQuery, callback_data: dict, session: AsyncSession):
    ...
    elif enter_index_product_in_cart < 0:
    ...
    if cart_products:
        ...
        await call.message.answer_photo(
            types.InputFile(await product.get_photo_file_id(session)),
            caption=_get_product_in_cart_caption(product, cart_products[enter_index_product_in_cart].quantity),
...
def _get_product_in_cart_caption(product: Product, quantity: int) -> str:
